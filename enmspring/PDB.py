...
class PDBReader:
    ...
    """
    def __init__(self, fname, skip_header=1, skip_footer=1, segid_exist=False, withfragid=False):
        self.fname = fname
        self.skip_header = skip_header
        self.skip_footer = skip_footer
        self.segid_exist = segid_exist
        if withfragid:
            self.atomgroup = self._read_pdb_with_fragid()
        else:
    ...
    def _read_pdb_with_fragid(self):
        lines = np.genfromtxt(self.fname, skip_header=self.skip_header, skip_footer=self.skip_footer, dtype=str)
        atomgroup = list()
        for line in lines:
            atomgroup.append(atom.AtomWithFragID(line, self.segid_exist))
        return atomgroup
